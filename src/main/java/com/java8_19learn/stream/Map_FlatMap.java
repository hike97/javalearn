...
public class Map_FlatMap {
	...
	@Test
	public void test_flatMap_print () {
		eggs.stream ().flatMap(x -> Arrays.stream(x).map(y -> y.replace("鸡", "煎"))).forEach (System.out::println);
		eggs.stream ().map(x -> Arrays.stream(x).map(y -> y.replace("鸡", "煎"))).forEach (System.out::println);
	}
